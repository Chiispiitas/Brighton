...
(() => {
  const STORAGE_KEY = "brighton-a1-rw-exam-state-v1";
  const EXAM_ID = "brighton-a1-rw-final";
  const EXAM_TITLE = "Brighton A1 Reading and Writing Final Exam";
  const SKILL = "Reading and Writing";
  const LEVEL = "A1";
  const EXAM_TYPE = "reading-writing";
  const RUBRIC_PROFILE = "a1rw";
  ...
  function loadState() {
    try {
      ...
      console.warn("Could not load saved A1 exam state", error);
      return null;
    }
  }
            ...
            ${part.items.map(item => `
              <article class="question-card ${getCurrentQuestionNumber() === item.q ? "active" : ""}" data-question-card="${item.q}">
                <div class="person-text">
                  <h3>${escape(item.stem)}</h3>
                  <p class="muted-text">${escape(item.replyLabel || "Answer")}:</p>
                </div>
  ...
  function renderPartThree(part) {
    const titleItem = part.items.find(item => item.q === 17);
    return `
      ...
      <section class="exam-panel part-five">
        ...
        <div class="writing-layout">
          <article class="article-card">
            ...
            ${(part.paragraphs || []).map(paragraph => `<p>${escape(paragraph)}</p>`).join("")}
          </article>
            ...
            <div class="question-stack">
              ${part.items.map(item => {
                ...
                return `
                  <article class="question-card ${getCurrentQuestionNumber() === item.q ? "active" : ""}" data-question-card="${item.q}">
                    <h4><span class="q-badge">${item.q}</span> ${escape(item.stem)}</h4>
                    <input class="inline-input ${getCurrentQuestionNumber() === item.q ? "active" : ""}" value="${escapeAttr(answer)}" data-open-input data-part-id="${part.id}" data-q="${item.q}" maxlength="60" placeholder="1–3 words" aria-label="Question ${item.q}" />
                  </article>`;
              }).join("")}
            </div>
          </aside>
            ...
            <div class="question-stack">
              ${part.items.map(item => {
                ...
                return `
                  <article class="question-card ${getCurrentQuestionNumber() === item.q ? "active" : ""}" data-question-card="${item.q}">
                    <h4><span class="q-badge">${item.q}</span> ${escape(item.stem)}</h4>
                    <input class="inline-input ${getCurrentQuestionNumber() === item.q ? "active" : ""}" value="${escapeAttr(answer)}" data-open-input data-part-id="${part.id}" data-q="${item.q}" maxlength="60" placeholder="Write your answer" aria-label="Question ${item.q}" />
                  </article>`;
              }).join("")}
            </div>
          </aside>
  ...
  function bindMainEvents(part) {
    ...
    if (part.id === "part2" && dom.mainContent.querySelector(".split-grid[data-resizable='part2']")) attachDivider("part2", ".split-grid[data-resizable='part2']", "part2Left");
    if (part.id === "part3" && dom.mainContent.querySelector(".split-grid[data-resizable='part3']")) attachDivider("part3", ".split-grid[data-resizable='part3']", "part3Left");
  }
  ...
  function buildWritingSamples() {
    const part = examParts.find(candidate => candidate.id === "part6");
    if (!part) return [];
    return part.items
      ...
      .map(item => {
        ...
        return {
          ...
          title: item.q === 34 ? "Picture sentence 1" : "Picture sentence 2",
          targetReader: "Exam reader",
          prompt: item.stem || "Write one complete sentence about the picture.",
          answer,
  ...
  async function submitPayload(payload) {
    ...
    try {
      ...
      resultBox.innerHTML = `
        <p class="submit-error">Save error: ${escape(error.message || String(error))}</p>
