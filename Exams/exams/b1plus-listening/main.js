...
(() => {
  ...
  const STORAGE_KEY = "brighton-b1plus-listening-exam-state-v1";
  ...
  async function playAudioAndEnter() {
    ...
    try {
      ...
      dom.audioGateError.textContent = "The audio could not start. Check that exams/b1plus-listening/audio.mp3 exists, then try again.";
  ...
  function renderGapPart(part) {
    ...
    return `
      <section class="exam-panel part-gap part3">
        ${partHeader(part)}
        ${instruction(part.instruction)}
        <article class="article-card listening-gap-card speakers-club-card">
  ...
  async function submitPayload(payload) {
    const message = { type: "BRIGHTON_B1PLUS_LISTENING_SUBMIT", payload };
    try {
  ...
  function scrollActiveCardIntoView() {
    const active = dom.mainContent.querySelector(".question-card.active, .listening-gap-line.active");
    if (!active) return;
